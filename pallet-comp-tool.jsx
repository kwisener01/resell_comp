...
const EBAY_APP_ID = ""; // User pastes their eBay App ID here

async function fetchEbayData(query, userCost) {
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1000,
      system: `You are a resell comp engine. The user gives you a product name and their cost per unit. 
You simulate realistic eBay market data for that product and return ONLY valid JSON — no markdown, no explanation.

Return this exact structure:
{
  "product": "clean product name",
  "soldListings": [
    { "title": "...", "price": 00.00, "type": "sold" },
    { "title": "...", "price": 00.00, "type": "sold" },
    { "title": "...", "price": 00.00, "type": "sold" }
  ],
  "activeListings": [
    { "title": "...", "price": 00.00, "type": "active" },
    { "title": "...", "price": 00.00, "type": "active" }
  ],
  "avgSoldPrice": 00.00,
  "avgActivePrice": 00.00,
  "sellThroughRate": 0.00,
  "category": "Electronics|Toys|Home Goods|Tools|Clothing|General"
}

Make the prices realistic for actual eBay sold listings. sellThroughRate is a decimal 0-1.`,
      messages: [
        {
          role: "user",
          content: `Product: "${query}" — My cost per unit: $${userCost}. Give me realistic eBay comp data.`
        }
      ]
    })
  });
...
function getVerdict(roi, sellThrough) {
  if (roi >= 40 && sellThrough >= 0.6) return { type: "buy", icon: "✅", label: "BUY IT", reason: `Strong ROI + ${Math.round(sellThrough * 100)}% sell-through rate` };
  if (roi >= 20 && sellThrough >= 0.4) return { type: "maybe", icon: "⚠️", label: "MAYBE", reason: `Acceptable margin but slower sell-through` };
  return { type: "pass", icon: "❌", label: "PASS", reason: `ROI too thin or slow market — not worth the risk` };
}

export default function PalletComp() {
  ...
  const run = useCallback(async () => {
    ...
    setResult(null);
    try {
      const data = await fetchEbayData(query.trim(), parseFloat(cost));
      ...
      setError("Could not analyze this product. Check your input and try again.");
    } finally {
      setLoading(false);
    }
  }, [query, cost]);
  ...
  const roiColor = result
    ? result.margin.roi >= 40 ? "green"
    : result.margin.roi >= 20 ? "yellow"
    : "red"
    : "white";
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="api-note">
          ⚡ DEMO MODE — Using AI-simulated eBay comps. To wire live eBay data, paste your free eBay App ID into EBAY_APP_ID in the source. Get one at developer.ebay.com → it's free.
        </div>
          ...
          <>
            ...
            <div className="breakdown">
              ...
              <div className="breakdown-row total">
                ...
                <span className="val">{result.margin.net >= 0 ? "+" : ""}${result.margin.net.toFixed(2)} ({result.margin.roi.toFixed(0)}% ROI)</span>
              </div>
            </div>
          </>
        )}
        ...
        <div className="disclaimer">
          COMPSCAN v1.0 — Demo uses AI-simulated market data. For live accuracy, connect the eBay Browse API (sold listings) and Keepa API (Amazon BSR). 
